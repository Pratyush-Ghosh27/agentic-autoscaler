...
- `internal/classifier/params.go:19` `KTodDown = 0.5` — F13 spec name is `K_PERIODIC_DOWN`. Spec acknowledges this divergence; rename in code is tracked separately. The naming is inconsistent with the spec and with the `tod_correlation`-was-renamed-to-`hourly_autocorr` rename that already happened in `Features.TodCorrelation`. (Field-level rename `Features.TodCorrelation → Features.HourlyAutocorr` landed post-Phase-6 alongside the constant rename `KTodDown → KPeriodicDown`; the historical narrative above is preserved as the audit trail of why the divergence existed.)
...
**Code reality.**
- `internal/classifier/params.go:83-87` selector is `if f.HourlyAutocorr > 0.70 OR |f.TrendSlope| > 2.0 → prophet else linear_extrap` (the field was named `f.TodCorrelation` at the time this gap was logged; renamed post-Phase-6). Never picks `gbdt_quantile`. Picks `prophet` for `gradual_ramp` (when |slope| > 2.0) — the wrong forecaster per the v2 mapping. Picks `linear_extrap` for some `spiky` workloads (when `tod ≤ 0.70` and `|slope| ≤ 2.0` but `cv > 0.50` and `peak_to_trough > 5`) — also wrong.
