...
.PHONY: k6-incluster-stress
k6-incluster-stress: ## Hackathon-three: 60-min run with 6× (7m baseline @ 200 RPS, 3m spike @ 600 RPS) — designed to maximise 503-rate gap between predictive and reactive. Set STRESS_CYCLES to change.
	bash deploy/k6/run-incluster.sh stress
...
.PHONY: k6-stress
k6-stress: ## Run the stress k6 scenario from the host (single-pod; debug only).
