...
type ClassifiedParams struct {
	...
	// All three are reachable from the classifier's pattern → forecaster
	// table (internal/classifier/params.go, G19): periodic → prophet,
	// spiky → gbdt_quantile, flat/gradual_ramp/default → linear_extrap.
	// F22 constrains only the Forecast Service's length-based auto branch
	// (rule 3 in forecast-service/src/forecast/dispatch.py), which never
	// picks gbdt_quantile when preferred_model is empty; an explicit spec
	// pin or a "spiky" classification is the way in.
